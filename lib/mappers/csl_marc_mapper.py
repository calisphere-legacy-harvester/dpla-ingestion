...
logger.error('this has to be here for some reason')
...
class CSLMARCMapper(PyMARCMapper):
    def __init__(self, provider_data):
        ...
        self.mapping_dict.update({
            lambda t: t == "856": [(self.map_is_shown_by, "bt")]
        })
        self.mapping_dict.update({
    ...
    def map_is_shown_by(self, _dict, tag, codes):
        prop = "isShownBy"
        if tag == '856':
            logger.error(self._get_values(_dict, codes))
            if '0' in _dict:
                urlString = self._get_values(_dict, codes)[0]
                imgString = self._get_values(_dict, codes)[1]
                self.mapped_data[prop] = ''.join(
                    ('http://catalog.library.ca.gov', urlString, '/',
                     imgString))

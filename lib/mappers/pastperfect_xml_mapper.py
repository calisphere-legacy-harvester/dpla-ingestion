...
class PastPerfectXMLMapper(Mapper):
    ...
    def map_is_shown_at(self, index=None):
        ...
        if 'thumbnail' in self.metadata:
            ...
            if '.tif' in self.metadata.get('thumbnail')[0]:
                jpg_url = self.metadata.get('thumbnail')[0].replace(".tif", ".jpg")
                self.mapped_data.update({"isShownBy": jpg_url})
            else:
                self.mapped_data.update({"isShownBy": self.metadata.get('thumbnail')[0]})
    ...
    def map_title(self):
        if 'title' in self.metadata:
            self.update_source_resource({'title': self.metadata.get('title')})

    def map_date(self):
        if 'date' in self.metadata:
            self.update_source_resource({'date': self.metadata.get('date')})
    ...
    def map_place(self):
        if 'place' in self.metadata:
            self.update_source_resource({
                'spatial': self.metadata.get('place')
                })

    def map_temporal(self):
        if 'coverage' in self.metadata:
            self.update_source_resource({
                'temporal': self.metadata.get('coverage')
                })

    def map_format(self):
        formats = []
        if 'medium' in self.metadata:
            formats.append(self.metadata.get('medium')[0])
        if 'material' in self.metadata:
            formats.append(self.metadata.get('material')[0])
        if formats:
    ...
    def map_creator(self):
        ...
        if 'creator' in self.metadata:
            creators.append(self.metadata.get('creator')[0])
        if 'author' in self.metadata:
            creators.append(self.metadata.get('author')[0])
        if 'artist' in self.metadata:
            creators.append(self.metadata.get('artist')[0])
        if 'photographer' in self.metadata:
            creators.append(self.metadata.get('photographer')[0])
        if creators:
    ...
    def map_identifier(self):
        ...
        if 'identifier' in self.metadata:
            identifiers.append(self.metadata.get('identifier')[0])
        if 'objectid' in self.metadata:
            identifiers.append(self.metadata.get('objectid')[0])
        if 'arkid' in self.metadata:
            identifiers.append(self.metadata.get('arkid')[0])
        if identifiers:
    ...
    def map_type(self):
        if 'objectname' in self.metadata:
            self.update_source_resource({
                'type': self.metadata.get('objectname')
                })

    def map_relation(self):
        if 'relation' in self.metadata:
            self.update_source_resource({
                'relation': self.metadata.get('collection')
                })

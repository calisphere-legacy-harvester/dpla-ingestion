...
class UCB_BAMPFA_Mapper(Mapper):
    ...
    def map_is_shown_at(self):
        ...
        is_shown_at = ''.join(('https://webapps.cspace.berkeley.edu/bampfa/search/search/?idnumber=', id_local, '&displayType=full&maxresults=1&start=1'))
    ...
    def map_is_shown_by(self):
        ...
        blob = blobArray[0]
        is_shown_by = ''.join(('https://webapps.cspace.berkeley.edu/bampfa/imageserver/blobs/', blob, '/derivatives/Medium/content'))

        self.mapped_data.update({"isShownBy": is_shown_by})

    def map_creator(self):
        self.update_source_resource({'creator': self.metadata.get('artistcalc_s')})
    ...
    def map_identifier(self):
        self.update_source_resource({'identifier': self.metadata.get('idnumber_s')})

    def map_genre(self):
        self.update_source_resource({'genre': self.metadata.get('itemclass_s')})

    def map_format(self):
        self.update_source_resource({'format':
        self.metadata.get('materials_s')})

    def map_provenance(self):
        self.update_source_resource({'provenance':
        self.metadata.get('creditline_s')})
